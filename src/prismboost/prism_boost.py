"""PrismBoost: gradient boosting with oblique splits from SEFR at each internal node.

The estimators here are :class:`PrismBoostClassifier` and
:class:`PrismBoostRegressor`. ``SEFRBoost*`` and ``SEFRGradientBoosting*`` are
kept as aliases for the names used before the project was renamed.
"""
...
__all__ = [
    ...
    "PrismBoostRegressor",
    # Legacy names, kept as aliases of the PrismBoost classes.
    "SEFRBoostClassifier",
    "SEFRBoostRegressor",
...
class PrismBoostClassifier(ClassifierMixin, BaseEstimator):
    ...
    matching the LinearBoost gradient-boosting formulation. Leaves output a
...
'hybrid'}, default='auto'
        ...
        axis-aligned single-feature splits.
...
class PrismBoostRegressor(RegressorMixin, BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if y.ndim == 0:
            ...
            if y.shape[1] != 1:
                raise ValueError(
                    "PrismBoostRegressor only supports single-target regression."
                )
            y = y.ravel()
        elif y.ndim != 1:
            raise ValueError(
...
# Legacy names from before the PrismBoost rename. They are the same objects, so
# ``isinstance`` checks and pickles written under the old names keep working.
SEFRBoostClassifier = PrismBoostClassifier
SEFRBoostRegressor = PrismBoostRegressor
SEFRGradientBoostingClassifier = PrismBoostClassifier
SEFRGradientBoostingRegressor = PrismBoostRegressor
